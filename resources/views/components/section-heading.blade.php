<div class="xl:flex xl:justify-between xl:items-center">
    ...
    <div x-data="{ dropdowns: false, activeItem: 0 }" class="relative">
        <div class="flex mt-4 xl:mt-0 xl:ml-4">
            <div class="relative grow z-10">
                ...
                <x-input x-model="searchFilter" type="text" class="h-full pl-12 rounded-none rounded-l-md" placeholder="Buscar..." />
            </div>
        ...
        <ul x-show="dropdowns" x-transition.origin.top.right x-on:click.outside="dropdowns = false" role="list" class="absolute z-10 right-0 overflow-hidden w-72 mt-2 bg-white ring-1 ring-black/[0.1] rounded-md shadow-lg" style="display: none;">
            <li x-on:click="dropdowns = false; activeItem = 0; categoryFilter = ''" :class="{ 'bg-indigo-600 text-white': activeItem === 0 }" class="p-4 text-lg text-gray-900 hover:text-white hover:bg-indigo-600">
                <div class="flex justify-between items-center">
                    <p class="font-semibold">Mostrar todos</p>
                    <span x-show="activeItem === 0" :class="{ 'text-white': activeItem === 0 }" class="text-indigo-600">
