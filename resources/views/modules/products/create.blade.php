        ...
        <div class="py-5 px-6">
            ...
            <div x-data="{ price: null, discount: null }" class="grid grid-cols-1 gap-x-6 gap-y-8 md:grid-cols-6">
                <div class="md:col-span-1">
                    <x-label for="create-code" value="{{ __('Código') }}" />
                    <x-input id="create-code" class="block mt-1 w-full" type="text" name="code" required />
                </div>
                ...
                <div class="md:col-span-2">
                    <x-label for="create-reference" value="{{ __('Referencia') }}" />
                    <x-select id="create-reference" class="block mt-1 w-full" name="reference" required>
                ...
                <div class="md:col-span-2">
                    <x-label for="create-category" value="{{ __('Categoría') }}" />
                    <x-select id="create-category" class="block mt-1 w-full" name="category" required>
                        <option value="" disabled selected hidden>Seleccione una categoría</option>
                ...
                <div class="md:col-span-2">
                    <x-label for="create-provider" value="{{ __('Proveedor') }}" />
                    <x-select id="create-provider" class="block mt-1 w-full" name="provider" required>
                ...
                <div class="md:col-span-2">
                    <x-label for="create-service" value="{{ __('Servicio') }}" />
                    <x-select id="create-service" class="block mt-1 w-full" name="service" required>
                ...
                <div class="md:col-span-1">
                    <x-label for="create-existence" value="{{ __('Existencia') }}" />
                    <x-input id="create-existence" class="block mt-1 w-full" type="text" name="existence" required />
                </div>
                <div class="md:col-span-2">
                    <x-label for="create-price" value="{{ __('Precio base') }}" />
                    <x-input x-model="price" id="create-price" class="block mt-1 w-full" type="text" name="price" required />
                </div>
                <div class="md:col-span-1">
                    <x-label for="create-discount" value="{{ __('Descuento') }}" />
                    <x-input x-model="discount" id="create-discount" class="block mt-1 w-full" type="text" name="discount" required />
                </div>
                <div class="md:col-span-full">
                    <p class="text-lg font-semibold text-gray-600">Precio final: <span x-text="(price - (price * discount / 100)).toLocaleString()"></span></p>
                </div>
                <div class="md:col-span-full">
                    <x-label for="create-description" value="{{ __('Descripción') }}" />
                    <x-textarea id="create-description" class="block mt-1 w-full" name="description" required />
                </div>
                <div class="md:col-span-full">
                    <x-label for="create-image" value="{{ __('Imagen') }}" />
                    <div class="flex justify-center mt-2 py-10 px-6 rounded-lg border-gray-900/[0.25] border-4 border-dashed">
                        <div class="text-center">
                            <img id="image" class="mb-3 rounded-lg" style="display: none;">
                            <svg id="icon" fill="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" class="w-16 h-16 mb-3 mx-auto text-gray-300">
                            ...
                            <x-label for="create-image" class="text-indigo-600 cursor-pointer hover:text-indigo-500">
                                <span>Cargar un archivo</span>
                                <x-input id="create-image" class="sr-only" type="file" name="image" accept="image/*" />
                            </x-label>
