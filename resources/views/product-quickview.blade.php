        ...
        <div class="relative flex items-center w-full pt-14 px-4 pb-8 md:pt-8 md:px-6 xl:p-8">
            ...
            <div class="grid grid-cols-1 gap-x-6 gap-y-8 items-start w-full md:grid-cols-12 xl:gap-x-8">
                ...
                <div class="md:col-span-8 xl:col-span-7">
                    ...
                    <section class="mt-3">
                        <p class="text-2xl font-medium text-gray-900">${{ number_format($product->sale_price, 0, '.', '.') }}</p>
                        <p class="mt-6 text-lg text-gray-700">{!! nl2br($product->description) !!}</p>
                    </section>
                    <section class="mt-6">
                            @if ($product->existence > 0)
                            <div class="mt-6">
                                <x-button class="inline-flex justify-center w-full py-3 px-8" onclick="agregarProducto('{{ $product->name }}', '{{ $product->sale_price }}')" x-on:click="Livewire.emit('addCarrito', {{ $product->id }})">
                            ...
                            @else
                                <p class="text-center font-semibold text-lg">Ahora mismo no tenemos existencia de este producto</p>
                            @endif
